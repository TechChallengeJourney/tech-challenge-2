...
export function BytebankGeneralCardsWidget() {
  ...
  const [widgetsData, setWidgetsData] = useState<
    Partial<BytebankGeneralWidgetCardProps>[]
  >([]);
  const userId = user?._id ?? "";
  const selectedWidgets = user?.selectedWidgets ?? [];
  const cardWidgets = [
    WidgetKey.HighestIncome,
    WidgetKey.MostExpensiveCategory,
    WidgetKey.DailyAverage,
  ];

  const filteredWidgets = cardWidgets.filter((widget) =>
    ...
    const fetchAllWidgets = async () => {
      ...
      if (userId) {
        ...
        try {
          const results = await Promise.all(
            filteredWidgets.map(async (widgetType) => {
              const data = await fetchWidgetData<
                Partial<BytebankGeneralWidgetCardProps>
              >(widgetType, userId);
              const widgetSelected = widgets.find(
                (widget) => widget.key === widgetType
              );
          ...
          setWidgetsData(
            results.filter(
              (item): item is Partial<BytebankGeneralWidgetCardProps> =>
                item !== null
            )
          );
        } catch (error) {
            ...
            <BytebankCard key={widget.key}>
              <Box padding={2}>
                <Box display="flex" alignItems="center" gap={2}>
                  {widget.icon}
                  <BytebankText variant="sm" color="primary">
                    {widget.label}
                  </BytebankText>
                </Box>
                {widget.category ? (
                  <>
                    ...
                    <BytebankText variant="sm">
                      ...
                      <span style={{ fontWeight: "700" }}>
                        {formatCurrencyBRL(widget?.value ?? 0)}
                      </span>
                    </BytebankText>
                  </>
                ) : (
